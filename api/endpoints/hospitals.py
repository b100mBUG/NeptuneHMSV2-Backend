...
async def fetch_specific_hospital(hospital_id: int):
...
async def format_hospital(hospital_id: int, data: HospitalsEdit):
...
async def change_hospital_password(hospital_id: int, password_detail: PasswordChange):
...
async def renew_plan(hospital_id: int, activation_key: str):
