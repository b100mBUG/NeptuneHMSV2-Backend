...
async def add_worker(hospital_id: str, worker_detail: dict):
...
async def fetch_workers(hospital_id: str, sort_term: str, sort_dir: str):
    async with async_session.begin() as session:
...
async def search_workers(hospital_id: str, search_by: str, search_term: str):
    async with async_session.begin() as session:
        stmt = select(Worker).where(Worker.hospital_id == hospital_id)
        if search_by == "name":
...
async def get_specific_worker(hospital_id: str, worker_id: str):
    async with async_session.begin() as session:
...
async def signin(hospital_id: str, worker_detail):
    async with async_session.begin() as session:
        stmt = select(Worker).where(
            (Worker.hospital_id == hospital_id) &
...
async def edit_workers(hospital_id: str, worker_id: str, worker_detail: dict):
    async with async_session.begin() as session:
        stmt = select(Worker).where(
            (Worker.hospital_id == hospital_id)&
            (Worker.worker_id == worker_id)
...
async def delete_worker(hospital_id: str, worker_id: str):
    async with async_session.begin() as session:
        stmt = select(Worker).where(
            (Worker.hospital_id == hospital_id) &
            (Worker.worker_id == worker_id)
