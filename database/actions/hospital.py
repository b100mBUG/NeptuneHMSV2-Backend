...
async def renew_hospital_plan(hospital_id: str, activation_key: str):
...
async def get_specific_hospital(hospital_id: str):
    async with async_session() as session:
...
async def edit_hospital(hospital_id: str, hospital_detail: dict):
    async with async_session.begin() as session:
