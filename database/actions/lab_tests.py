...
async def add_lab_tests(hospital_id: str, test_detail: dict):
...
async def fetch_lab_tests(hospital_id: str, sort_term: str, sort_dir: str):
    async with async_session.begin() as session:
...
async def search_lab_tests(hospital_id: str, search_term: str):
    async with async_session.begin() as session:
...
async def get_specific_test(hospital_id: str, test_id: str):
    async with async_session.begin() as session:
...
async def edit_lab_test(hospital_id: str, test_id: str, test_detail: dict):
    async with async_session.begin() as session:
...
async def delete_lab_test(hospital_id: str, test_id: str):
    async with async_session.begin() as session:
        test = await get_specific_test(hospital_id, test_id)
        if not test:
            return None
