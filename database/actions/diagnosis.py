...
async def add_diagnosis(hospital_id: str, diagnosis_detail: dict):
...
async def fetch_diagnosis(hospital_id: str, sort_term: str, sort_dir: str):
...
async def search_diagnosis(hospital_id: str, search_term: str):
    async with async_session.begin() as session:
        stmt = (
            select(Diagnosis)
            .join(Patient)
...
async def get_specific_diagnosis(hospital_id: str, diagnosis_id: str):
    async with async_session.begin() as session:
        stmt = (
            select(Diagnosis)
            .where(
...
async def edit_diagnosis(hospital_id: str, diagnosis_id: str, diagnosis_detail: dict):
    async with async_session.begin() as session:
...
async def delete_diagnosis(hospital_id: str, diagnosis_id: str):
    async with async_session.begin() as session:
        diagnosis = await get_specific_diagnosis(hospital_id, diagnosis_id)
        if not diagnosis:
            return None
