...
async def add_appointment(hospital_id: str, appointment_detail: dict):
...
async def fetch_appointments(hospital_id: str, sort_term: str, sort_dir: str):
...
async def search_appointments(hospital_id: str, search_term: str):
    async with async_session.begin() as session:
        stmt = (
            select(Appointment)
            .join(Patient)
...
async def get_specific_appointment(hospital_id: str, appointment_id: str):
    async with async_session.begin() as session:
        stmt = (
            select(Appointment)
            .where(
...
async def edit_appointment(hospital_id: str, appointment_id: str, appointment_detail: dict):
    async with async_session.begin() as session:
...
async def delete_appointment(hospital_id: str, appointment_id: str):
    async with async_session.begin() as session:
        appointment = await get_specific_appointment(hospital_id, appointment_id)
        if not appointment:
            return None
